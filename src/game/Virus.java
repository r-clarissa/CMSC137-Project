...
public class Virus extends Sprite {
	...
	public final static Image VIRUS_REGULAR = new Image("images/Virus.png",Virus.VIRUS_WIDTH,Virus.VIRUS_WIDTH,false,false);
	public final static Image VIRUS_BULLET = new Image("images/Virus.png",60,60,false,false);
...
//		this.x -= speed;
 		if (this.getVisible()) {
 			if(this.moveRight == true){ // If moveRight is true and
				if(this.getX() < GameStage.WINDOW_WIDTH){ // if the fish hasn't reached the right boundary yet
					this.x += this.speed; // move the fish to the right by changing the x position of the fish depending on its speed
				}else if(this.getX() >= GameStage.WINDOW_WIDTH){ // else if it has reached the boundary, change the moveRight value / move to the left
