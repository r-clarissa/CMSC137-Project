...
public class Player extends Sprite{
	...
	public final static Image DOCTOR_IMAGE = new Image("images/Token 1.png",Player.DOCTOR_WIDTH,Player.DOCTOR_WIDTH,false,false);
	private final static int DOCTOR_WIDTH = 50;

	Player(String name, int x, int y, int tokenType){
		super(x,y);
		this.name = name;
		Random r = new Random();
		...
		this.score = 0;
		this.loadImage(SuperStage.getTokenImg(tokenType));
	}
	...
	void move() {
		 ...
		 *
		 */
		if(this.getX() < (SuperStage.WINDOW_WIDTH-75) && this.getX() > 0){ // Only change the x and y position of the ship if the current x,y position
			if(this.getY() < (SuperStage.WINDOW_HEIGHT-75) && this.getY() > 45){ // is within the gamestage width and height so that the ship won't exit the screen
				this.x += this.dx;
				this.y += this.dy;
			}else if(this.getY() < 75){
				this.y += 5;
			}else if(this.getY() >= (SuperStage.WINDOW_HEIGHT-100)){
				this.y -= 5;
			}
		}else if(this.getX() < 1){
			this.x += 5;
		}else if(this.getX() >= (SuperStage.WINDOW_WIDTH-100)){
			this.x -= 5;
		}
	}
}
