...
public class BigBoss extends Sprite {
	...
	public final static Image BIGVIRUS_IMAGE = new Image("images/Boss.png",175,175,false,false);
	public final static int BIGVIRUS_WIDTH=200;
	...
	BigBoss(GameTimer gamerTimer) {
		...
		this.alive = true;
		this.loadImage(BigBoss.BIGVIRUS_IMAGE);
		//this.speed ;
		this.life = 3000;
	...
	void move(){
		...
		if(this.moveDown == true){ // If moveDown is true and
			if(this.getY() < (GameStage.WINDOW_HEIGHT-190)){ // if the boss hasn't reached the right boundary yet
				this.y += this.speed; // move the boss to the up by changing the x position of the fish depending on its speed
				moveLeftRight();

			}else if(this.getY() >= (GameStage.WINDOW_HEIGHT-190)){ // else if it has reached the boundary, change the moveDown value / move up
				this.moveDown = false;
			}
	...
	private void moveLeftRight() {
		...
		if(this.diagonal == true){ // If diagonal is true and
			if(this.getX() < (GameStage.WINDOW_WIDTH-190)){ // if the fish hasn't reached the right boundary yet
				this.x += this.speed; // move the fish to the right by changing the x position of the fish depending on its speed
			}else if(this.getX() >= (GameStage.WINDOW_WIDTH-190)){ // else if it has reached the boundary, change the moveRight value / move to the left
				this.diagonal = false;
			}
	...
	void damagePlayer(Doctor player){
